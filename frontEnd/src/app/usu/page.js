...
import "../home.css";
import "./usu.css";
import NextJsCarousel from "../carrossel/page";
...
export default function Usu() {
  ...
  const sendWhatsAppMessage = () => {
    ...
    const productsList = cart
      .map(
        (product) => `Produto: ${product.titulo}, Preço: R$ ${product.preco}\n`
      )
      .join("");
    const totalPrice = `\n\nValor total: R$ ${totalValue}`;
    const whatsappMessage = encodeURIComponent(
      message + productsList + totalPrice
    );
    ...
    <main>
      <section className="main-content">
        <header className="header">
          ...
          {/* Adiciona botão para alternar entre mostrar e ocultar o carrinho */}
              ...
              {cart.length === 0 ? (
                ...
                <>
                  {cart.map((product) => (
                    <div key={product.id}>
                      <p>
                        {product.titulo} - R$ {product.preco}
                      </p>
                    </div>
                  ))}
                  <button onClick={sendWhatsAppMessage} className="button">
                    Comprar no WhatsApp
                  </button>
                </>
              )}
            ...
            <div className="produto-card" key={produto.id}>
              ...
              <div className="produto-conteudo">
                ...
                <div className="socorro">
                  <button onClick={() => addToCart(produto)} className="button">
